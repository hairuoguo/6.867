import numpy as np
import math
import loadFittingDataP1 as ld
...
def main():
    ...
    print(batch_gd(gd_x, gd_y, batch_step_size, batch_threshold,  avg_least_sq, avg_least_sq_d))
    ideal_theta = np.linalg.inv(gd_x.T.dot(gd_x)).dot(gd_x.T).dot(gd_y)
    print(avg_least_sq(gd_x, gd_y, ideal_theta))
    print(stochastic_gd(gd_x, gd_y, batch_step_size, batch_threshold,  avg_least_sq, avg_least_sq_d))
...
def avg_least_sq(batch_x, batch_y, theta):
    return np.sum((batch_x.dot(theta) - batch_y)**2)/batch_x.shape[0]    

def avg_least_sq_d(batch_x, batch_y, theta):
    return 2*batch_x.T.dot(batch_x.dot(theta) - batch_y)/batch_x.shape[0]
...
def batch_gd(data_x, data_y, step_size, threshold, obj_func, d_func):
    theta = np.random.normal(0, 0.1, data_x.shape[1])
    prev_error = obj_func(data_x, data_y, theta)
    convergence = False
    while not convergence:
        gradient = d_func(data_x, data_y, theta)
        theta = -1*gradient*step_size + theta
        error = obj_func(data_x, data_y, theta)
        if math.isnan(error):
            return
...
def stochastic_gd(data_x, data_y, step_size, threshold, obj_func, d_func):
    theta = np.random.normal(0, 0.1, data_x.shape[1])
    prev_error = obj_func(data_x, data_y, theta)
    convergence = False
    datum_count = 0
